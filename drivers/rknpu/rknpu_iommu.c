...
#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE && LINUX_VERSION_CODE < KERNEL_VERSION(6, 18, 0)
...
int rknpu_iommu_switch_domain(struct rknpu_device *rknpu_dev, int domain_id)
{
	...
	if (dst_domain != NULL) {
		...
		/* Use new 6.18.2 API: iommu_paging_domain_alloc() replaces iommu_domain_alloc() */
