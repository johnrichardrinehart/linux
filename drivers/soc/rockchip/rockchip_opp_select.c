				      ...
				      struct pvtm_config *pvtm)
{
	...
	if (IS_ERR(pvtm->tz))
		return -EINVAL;
	/* thermal_zone_get_temp() will return error if get_temp not available */
			     ...
			     bool scaling_down, struct rockchip_opp_info *info)
{
	...
	/* Note: rate_clk_single is internal to OPP, cannot access from outside */
	dev_dbg(dev, "Setting clock rate to %lu (Hz)\n", *target);
	ret = clk_set_rate(opp_table->clk, *target);
	if (ret)
		dev_err(dev, "failed to set clock rate: %lu\n", *target);
	/* opp_table->rate_clk_single = *target; - internal OPP struct, not accessible */
