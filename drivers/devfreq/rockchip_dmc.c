...
/*
 * Stub functions for DRM coordination (optional feature)
 * Real implementations exist in rockchip DRM driver
 */
#ifndef CONFIG_ROCKCHIP_DRM
static inline u32 rockchip_drm_get_sub_dev_type(void)
{
	return 0;
}
...
#else
extern u32 rockchip_drm_get_sub_dev_type(void);
extern u32 rockchip_drm_get_scan_line_time_ns(void);
#endif
...
int rockchip_dmcfreq_wait_complete(void)
{
	...
	if (wait_ctrl.dcf_en == 1) {
		...
		res = sip_smc_dram(0, 0, ROCKCHIP_SIP_CONFIG_DRAM_MCU_START);
		if (res.a0) {
...
EXPORT_SYMBOL(rockchip_dmcfreq_wait_complete);
...
/*
 * Disabled: requires access to internal OPP structures not exposed in public API
 */
#if 0
...
#endif /* 0 - rockchip_dmcfreq_adjust_opp_table disabled */
					  ...
					  struct rockchip_dmcfreq *dmcfreq)
{
	...
	/*
	 * Disabled: rockchip_dmcfreq_adjust_opp_table requires access to
	 * internal OPP structures not exposed in public API
	 */
#if 0
	...
	dmcfreq->is_set_rate_direct = true;
#endif /* 0 - OPP table adjustment disabled */
	...
	}
	{
		...
		ret = dev_pm_opp_get_supplies(opp, supplies);
		if (ret) {
			dev_err(&pdev->dev, "Failed to get OPP supplies\n");
			dev_pm_opp_put(opp);
			return ret;
		}
		dmcfreq->sleep_volt = supplies[0].u_volt;
		if (dmcfreq->opp_info.regulator_count > 1)
			dmcfreq->sleep_mem_volt = supplies[1].u_volt;
	}
	dev_pm_opp_put(opp);
