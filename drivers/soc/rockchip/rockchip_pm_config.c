...
/*
 * Compatibility wrappers for deprecated OF GPIO API (removed in 6.6+)
 */
enum of_gpio_flags {
...
static int of_get_named_gpio_flags(const struct device_node *np,
				   const char *propname, int index,
				   enum of_gpio_flags *flags)
{
	/*
	 * Modern kernel doesn't provide flag information via of_get_named_gpio.
	 * Just return the GPIO number and clear flags.
	 */
	if (flags)
		*flags = 0;
...
/*
 * Regulator on/off control during suspend requires internal regulator API.
 * Disabled for mainline kernel - use regulator suspend states in DT instead.
 */
...
#if 0  /* Regulator on/off control disabled - requires internal regulator API */
...
#if 0  /* Requires internal regulator API - disabled */
...
#endif /* 0 - parse_on_off_regulator_dev disabled */
...
#if 0  /* parse_on_off_regulator_dev disabled - requires internal regulator API */
...
#if 0  /* Variables for regulator control - disabled code below */
	int i;
